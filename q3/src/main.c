...
void startProduction(){
  ...
  task->jobQF = queueCreate(num_cars * 2);
  sem_init(&task->pick, 0, 1);
  task->typeA = num_typeA;
  task->typeB = num_typeB;
  task->typeC = num_typeC;
  // sem_init(&task->typeA, 0, num_typeA);
  // sem_init(&task->typeB, 0, num_typeB);
  // sem_init(&task->typeC, 0, num_typeC);
  int i;
  // for(int k = 0; k < num_cars; k++){
  //   i = 6;
  //   queueEnqueue(task->jobQF, &i);
  //   i = 7;
  //   queueEnqueue(task->jobQF, &i);
  // }
  for (int k = 0; k < num_cars; k++){
